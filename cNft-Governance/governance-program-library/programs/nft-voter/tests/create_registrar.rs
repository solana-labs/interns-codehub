...
async fn test_create_registrar_with_invalid_realm_authority_error() -> Result<(), TransportError> {
    ...
    // Act
    let err = nft_voter_test.with_registrar(&realm_cookie).await.err().unwrap();
...
#[tokio::test]
async fn test_create_registrar_with_realm_authority_must_sign_error() -> Result<
    (),
    TransportError
> {
    // Arrange
...
#[tokio::test]
async fn test_create_registrar_with_invalid_spl_gov_program_id_error() -> Result<
    (),
    TransportError
> {
    // Arrange
    ...
    let err = nft_voter_test
        .with_registrar_using_ix(
            &realm_cookie,
            |i| {
                i.accounts[1].pubkey = governance_program_id;
...
async fn test_create_registrar_with_invalid_realm_error() -> Result<(), TransportError> {
    ...
    let err = nft_voter_test
        .with_registrar_using_ix(
            &realm_cookie,
            |i| {
                i.accounts[2].pubkey = Pubkey::new_unique();
            }, // realm
            None
        ).await
        .err()
...
#[tokio::test]
async fn test_create_registrar_with_invalid_governing_token_mint_error() -> Result<
    (),
    TransportError
> {
    // Arrange
    ...
    let err = nft_voter_test
        .with_registrar_using_ix(
            &realm_cookie,
            |i| {
                i.accounts[3].pubkey = mint_cookie.address;
