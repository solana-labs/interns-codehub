...
pub mod cnft_voter {
    ...
    pub fn update_voter_weight_record<'a, 'b, 'c, 'info>(
        ...
        cnft_info_len: u32,
        params: Vec<utils::cnft_verification::VerifyParams2>,
    ) -> Result<()> {
        log_version();
    ...
    pub fn verify_compressed_nft_info<'a, 'b, 'c, 'info>(
        ...
        params: utils::cnft_verification::VerifyParams2,
    ) -> Result<()> {
        log_version();
        instructions::verify_compressed_nft_info(ctx, &params)
    }

    pub fn relinquish_compressed_nft_vote(
        ctx: Context<RelinquishCompressedNftVote>,
    ) -> Result<()> {
        log_version();
