...
async fn test_cast_cnft_vote() -> Result<(), TransportError> {
    ...
    let cnft_weight_record_address = get_cnft_weight_record_address(asset_id).0;

    let cnft_weight_record = cnft_voter_test.bench.get_account(&cnft_weight_record_address).await;
...
async fn test_cast_cnft_vote_with_multiple_trees() -> Result<(), TransportError> {
    ...
    let mut tree_cookie1 = cnft_voter_test.merkle_tree.with_merkle_tree(None).await?;
    let mut tree_cookie2 = cnft_voter_test.merkle_tree.with_merkle_tree(None).await?;
...
> {
    ...
    let mut tree_cookie2 = cnft_voter_test.merkle_tree.with_merkle_tree(Some(tree_size2)).await?;
