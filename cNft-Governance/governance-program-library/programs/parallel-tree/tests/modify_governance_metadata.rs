...
#[tokio::test]
async fn test_modify_governance_metadata() -> Result<(), TransportError> {
    ...
    // modify governance metadata to parallel tree
    let leaf_proof_cookie2 = parallel_tree_test.get_leaf_proof(
        &parallel_tree_cookie,
        &nft_leaf_cookie
    ).await?;

    parallel_tree_test.with_modify_governance_metadata(
        &parallel_tree_cookie,
        &nft_leaf_cookie,
        &leaf_proof_cookie2,
        &message,
