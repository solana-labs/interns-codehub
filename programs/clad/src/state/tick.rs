use {super::Globalpool, crate::errors::ErrorCode, anchor_lang::prelude::*};
...
impl Tick {
    ...
    /// Check that the tick index is a valid start tick for a tick array in this globalpool
    /// A valid start-tick-index is a multiple of tick_spacing & number of ticks in a tick-array.
    ///
    /// # Parameters
    /// - `tick_index` - A i32 integer representing the tick index
    /// - `tick_spacing` - A u8 integer of the tick spacing for this globalpool
    ///
    /// # Returns
    /// - `true`: The tick index is a valid start-tick-index for this globalpool
    /// - `false`: The tick index is not a valid start-tick-index for this globalpool
...
impl TickArray {
    ...
    ///
    /// # Parameters
    /// - `globalpool` - the tick index the desired Tick object is stored in
    /// - `start_tick_index` - A u8 integer of the tick spacing for this globalpool
    ///
    /// # Errors
    /// - `InvalidStartTick`: - The provided start-tick-index is not an initializable tick index in this Globalpool w/ this tick-spacing.
    pub fn initialize(
        &mut self,
        globalpool: &Account<Globalpool>,
        start_tick_index: i32,
    ) -> Result<()> {
        if !Tick::check_is_valid_start_tick(start_tick_index, globalpool.tick_spacing) {
...
impl TickLoan {
    pub const LEN: usize = std::mem::size_of::<TickLoan>();
}
