...
pub fn calculate_liquidity_token_deltas(
    ...
    if tick_current_index < tick_lower_index {
        ...
        delta_a = get_amount_delta_a(sqrt_lower_price, sqrt_upper_price, liquidity, round_up)?;
    } else if tick_current_index < tick_upper_index {
        // current tick inside position
        delta_a = get_amount_delta_a(sqrt_current_price, sqrt_upper_price, liquidity, round_up)?;
        delta_b = get_amount_delta_b(sqrt_lower_price, sqrt_current_price, liquidity, round_up)?;
    } else {
        // current tick above position
        delta_b = get_amount_delta_b(sqrt_lower_price, sqrt_upper_price, liquidity, round_up)?;
    }
